...
| `config_schema`     | no       | flat list of fields: `key` (`[a-z0-9_]`, unique), `type` (`string` \| `number` \| `boolean` \| `select` \| `multi_select` \| `secret` \| `oauth2`), `label` (multi-language, `en` mandatory), `description`, `placeholder` (multi-language, `string`/`number`/`secret` only), `required`, `default` (matches the field type, forbidden on `secret`/`oauth2`), `min`/`max` (number only), `options` (select/multi_select only), `display` (`dropdown` \| `radio`, select only)                      |
...
| `network_discovery` | no       | 1–5 mediated capture requests: `udp-broadcast` (1–5 unique `ports`), `mdns` (DNS-SD `service`, e.g. `_hue._tcp`) or `ssdp` (`st`, ≤ 200 characters) — each type only carries its own field                                                                                                                                                                                                                                                                                                         |
